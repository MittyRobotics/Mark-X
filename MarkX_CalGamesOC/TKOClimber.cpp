...
TKOClimber::TKOClimber(int port1, int port2) :
	_stick3(3), _stick4(4), sDumperR(PN_S1R_ID), sDumperE(PN_S1E_ID), 
...
{
	log->addMessage("CALIB: Starting winch calibration");
	winch1PID.Disable();
	winch2PID.Disable();
	RatchetBack();
	while (ds->IsEnabled())
	...
	log->addMessage("CALIB: Done going up, at the top.");
	while (ds->IsEnabled())
	{
	...
	log->addMessage("CALIB: Done going down, at the bottom.");
	winch1.Set((-1) * MAXSPEED); //go up
	winch2.Set((-1) * MAXSPEED);
	Wait(.5);
	log->addMessage("CALIB: At start point.");
	winch1.Set(0);
	winch2.Set(0);
	Wait(.5);
...
{
	DSClear();
//	dumpProtect.StartLiveWindowMode();
	winch1PID.Enable();
	winch2PID.Enable();
	...
	if (_stick4.GetRawButton(10))
	{
		dumpProtect.Set(0.);
		DSLog(2, "Dump protector set to angle 0\n");
	}
	if (_stick4.GetRawButton(11))
	{
		dumpProtect.Set(0.5);
		DSLog(2, "Dump protector set to angle 90\n");
	}
	if (_stick3.GetRawButton(10))
	{
		climbStart = ds->GetMatchTime();
		log->addCMessage("LVL 1 Climb started at (s)", climbStart);
		TKOClimber::LevelOneClimb();
		climbEnd = ds->GetMatchTime();
		climbTime = climbEnd - climbStart;
		log->addMessage("Done with level one climb");
		log->addCMessage("LVL 1 Climb took (s)", climbTime);
		printf("LVL 1 Climb took (s) %f\n", climbTime);
	}
	if (_stick3.GetRawButton(7))
	{
		climbStart = ds->GetMatchTime();
		log->addCMessage("LVL 2 Climb started at (s)", climbStart);
		if (TKOClimber::LevelOneClimb())
			TKOClimber::LevelTwoOrMoreClimb();
		climbEnd = ds->GetMatchTime();
		climbTime = climbEnd - climbStart;
		log->addMessage("Done with level two climb");
		log->addCMessage("LVL 2 Climb took (s)", climbTime);
		printf("LVL 2 Climb took (s) %f\n", climbTime);
	}
	if (_stick3.GetRawButton(6))
	{
		climbStart = ds->GetMatchTime();
		log->addCMessage("LVL 3 Climb started at (s)", climbStart);
		if (TKOClimber::LevelOneClimb())
		{
			TKOClimber::LevelTwoOrMoreClimb();
			Wait(.1);
		...
		climbEnd = ds->GetMatchTime();
		climbTime = climbEnd - climbStart;
		log->addMessage("Done with level three climb");
		log->addCMessage("LVL 3 Climb took (s)", climbTime);
		printf("LVL 3 Climb took (s) %f\n", climbTime);
	}
	if (comp.GetPressureSwitchValue() == 0)
		DSLog(1, "NOT max pressure.");
	DSLog(3, "SERVO ANGLE %f", dumpProtect.GetAngle());
	DSLog(4, "SERVO POS %f", dumpProtect.Get());
//	if (not clipLeft.Get())
...
{
	log->addMessage("LVL 1 Starting climb");
	climbCount = 0;
	if (ranCalibration == false)
	{
		log->addMessage("LVL 1 Climb called without calibration...");
		calibrateWinch();
		MoveWinchWithStick();
		return false;
	}
	...
	log->addMessage("LVL 1: Winch should now be at climb start position.");
	...
	Wait(.1);
	log->addMessage("LVL 1: Arm moved forward towards pyramid.");
	...
	{ //while where you want it to go is below than its ramping setpoint
		...
		if (winchEncoder.PIDGet() >= 1600)
		{
			if (hookLeft.Get() or hookRight.Get())
			{
				...
				RatchetBack();
				log->addMessage(">>>CRITICAL LVL 1: IMPROPERLY ALLIGNED? WHEN STARTING FIRST PULL HOOKS DID NOT ATTACH");
				Wait(0.25);
				log->addMessage(">>>CRITICAL LVL 1: ABORTING LVL 1 CLIMB, RETURNING OUT OF CLIMB FUNCTION.");
	...
	log->addMessage("LVL 1: Finished pulling up to arm back setpoint");
	ArmBack();

	Wait(2.);
	log->addMessage("LVL 1: Arm moved back.");
	...
	log->addMessage("LVL 1: Finished pulling up to clip back setpoint");
	ClipBack();
	Wait(.5);
	log->addMessage("LVL 1: Clips moved back");
	...
	log->addMessage("LVL 1: Winch now at bottom limit switch");
	Wait(.25);
	...
	ClipForward();
	Wait(.5);
	log->addMessage("LVL 1: Clip is forward, transferring weight");
	...
	{ //while where you want it to go is below than its ramping setpoint
		...
		{
			log->addMessage("LVL 1: CLIPS ENGAGED");
			break;
		}
	...
	if (clipLeft.Get() or clipRight.Get())
	{
		...
		{
			DSLog(1, "Left Clip Engaged");
			log->addMessage("LVL 1: Left clip engaged");
		}
		if (not clipRight.Get())
		{
			DSLog(2, "Right Clip Engaged");
			log->addMessage("LVL 1: Right clip engaged");
		}
		if (not hookLeft.Get())
		{
			DSLog(3, "Left Hook Engaged");
			log->addMessage("LVL 1: Left hook engaged");
		}
		if (not hookRight.Get())
		{
			DSLog(4, "Right Hook Engaged");
			log->addMessage("LVL 1: Right hook engaged");
		}
		while ((clipLeft.Get() or clipRight.Get()) and ds->IsEnabled())
		{
			...
			log->addMessage("LVL 1: CLIPS NOT ENGAGED");
			Wait(5.);
		}
	}
	...
	log->addMessage("LVL 1: Weight transferred, moving hooks back up to top.");
	...
	log->addMessage("LVL 1: DONE WITH LVL 1 CLIMB");
